...
export class DiagnosticTargetService {
  ...
  create(data: DiagnosticTargetCreate): Observable<DiagnosticTarget> {
    return this.http
      .post<{ success: boolean; data: DiagnosticTarget }>(this.BASE_URL, data)
      .pipe(map((res) => res.data));
  }
  ...
  update(
    id: string,
    data: DiagnosticTargetUpdate,
  ): Observable<DiagnosticTarget> {
    return this.http
      .put<{
        success: boolean;
        data: DiagnosticTarget;
  ...
  delete(id: string): Observable<void> {
    return this.http
      .delete<{ success: boolean; message: string }>(`${this.BASE_URL}/${id}`)
      .pipe(map(() => void 0));
  }
  ...
  history(
    ...
    limit: number = 100,
    serialNumber?: string,
  ): Observable<DiagnosticTargetHistoryEntry[]> {
    ...
    return this.http
      .get<{
        success: boolean;
        data: DiagnosticTargetHistoryEntry[];
        count: number;
      }>(url)
      .pipe(map((res) => res.data));
  }
  ...
  analysis(
    id: string,
    days: number = 30,
  ): Observable<DiagnosticTargetAnalysis | null> {
    return this.http
      .get<{
        success: boolean;
        data: DiagnosticTargetAnalysis | null;
  ...
  overview(
    days: number = 7,
  ): Observable<{ data: DiagnosticOverview | null; message?: string }> {
    return this.http
      .get<{
        success: boolean;
        data: DiagnosticOverview | null;
        message?: string;
      }>(`${this.BASE_URL}/overview?days=${days}`)
      .pipe(map((res) => ({ data: res.data, message: res.message })));
  }

  /** Lista alertas de destinos degradados (ativos, resolvidos ou todos). */
  listAlerts(
    status: 'active' | 'resolved' | 'all' = 'active',
    limit: number = 50,
  ): Observable<DiagnosticAlert[]> {
    ...
    return this.http
      .get<{
        ...
        count: number;
      }>(`${this.BASE_URL}/alerts${params}`)
      .pipe(map((res) => res.data));
  }

  /** Reconhece um alerta de diagnóstico degradado (marca acknowledgedBy/At). */
  acknowledgeDiagnosticAlert(alertId: string): Observable<DiagnosticAlert> {
    return this.http
      .post<{
        success: boolean;
        data: DiagnosticAlert;
